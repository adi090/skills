...
//     <div
//       onClick={onToggle}
//       className={`w-5 h-5 border-2 rounded mr-2 cursor-pointer ${
//         completed ? "bg-yellow-400 border-yellow-400" : "border-gray-400"
//       }`}
//     >
//       {completed && (
//         <svg
...
//         "http://localhost:5000/api/skill-analyzer",
//         formData,
//         {
//           headers: {
//             "Content-Type": "multipart/form-data",
...
//         newAnalysisResult.skills_required_in_job.length;
//       const matchingSkillsCount = newAnalysisResult.matching_skills.length;
//       const newMatchRate = Math.round(
//         ((matchingSkillsCount + 1) / totalRequiredSkills) * 100
//       );
//       setMatchRate(newMatchRate);
...
//       const newMarketFit = Math.round(
//         ((skillsToImproveCount + 1) / totalRequiredSkills) * 100
//       );
//       setMarketFit(newMarketFit);

//       const newSkills = newAnalysisResult.skills_to_improve.map(
//         (skill, index) => ({
//           id: index + 1,
//           name: skill,
//           completed: false,
...
//         "skillsToImprove",
//         JSON.stringify(skillsToImproveCount)
//       );
//     } catch (error) {
//       console.error("Error during analysis:", error);
//       // Handle error (e.g., show an error message to the user)
...
//     try {
//       const response = await axios.post(
//         "http://localhost:5000/api/recommend_course",
//         { resource: skillName },
//         { headers: { "Content-Type": "application/json" } }
//       );
//       console.log("Received from backend:", response.data);
...
//           Analyze Your Resume and Job Description
//         </h2>
...
//               Matching Skills: {matchRate}%
//             </h3>
//             {/* <p className="text-white">Market Fit: {marketFit}%</p> */}
//             <p className="text-white">Skills to Improve: {skillsToImprove}</p>

//             <SkillsVisualization skillsData={skillsData} />

//             <h4 className="text-white text-lg mt-4">Matching Skills</h4>
...
//             <ul className="text-white">
//               {analysisResult.skills_to_improve.map((skill, index) => (
//                 <li key={index}>
//                   <SkillCheckbox
//                     skill={skill}
//                     completed={skills.some(
//                       (s) => s.name === skill && s.completed
//                     )}
//                     onToggle={() => toggleSkill(index)}
//                   />
//                   {/* <button 
//                     onClick={(e) => sendSkillToBackend(e, skill)} 
//                     disabled={loadingSkills[skill]}
//                     className="bg-violet-500 text-white py-1 px-4 rounded ml-2"
//                   >
//                     {loadingSkills[skill] ? 'Loading...' : 'Find Resources'}
//                   </button> */}
//                 </li>
//               ))}
//             </ul>
